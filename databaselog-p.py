...
# twisted.enterprise.util no longer exist in Twisted 12.0
...
class HomePage(resource.Resource):
    ...
    def _gotPosts(self, results, request):
        request.write("""
                      ...
                      <body>
                        <h1>MircroBlog</h1>
                        <i>Like a blog, but less useful</li>
    ...
    def _dbError(self, failure, request):
        ...
        request.write("Error feaching posts: %s" % failure.getErrorMessage())
        request.finish()
...
class SavePage(resource.Resource):
    ...
    def render(self, request):
        ...
        self.db.runOperation("Insert into posts (title, body) values (%s, %s)", (title, body)).addCallback(
    ...
    def _saveFailed(self, failure, request):
        request.setResponseCode(http.INTERNAL_SERVER_ERROR)
        request.write("Error saving record: %s" % (failure.getErrorMessage()))
        request.finish()


class RootResourse(resource.Resource):
    def __init__(self, dbConnection):
        resource.Resource.__init__(self)
...
if __name__ == "__main__":
    ...
    f = server.Site(RootResourse(dbConnection))
